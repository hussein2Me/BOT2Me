...
async def _(event):
    ...
    if input_str:
        logger.info("TODO: لم يتم تنفيذه بعد")
    else:
        if event.is_private:
            return False
        await event.edit("البحث في قوائم المشاركين.")
        p = 0
        async for i in borg.iter_participants(event.chat_id, filter=ChannelParticipantsKicked, aggressive=True):
            ...
            try:
                ...
                logger.warn("لمدة {} ثانية".format(ex.seconds))
                sleep(ex.seconds)
            except Exception as ex:
                await event.edit(str(ex))
            else:
                p += 1
        await event.edit("{}: {} غير محظور".format(event.chat_id, p))
...
async def _(event):
    ...
    if input_str:
        ...
        if not (chat.admin_rights or chat.creator):
            await event.edit("`أنت لست مسؤولاً هنا!`")
            return False
    p = 0
    b = 0
    c = 0
    ...
    q = 0
    r = 0
    await event.edit("البحث في قوائم المستخدمين")
    async for i in borg.iter_participants(event.chat_id):
        ...
        if isinstance(i.status, UserStatusEmpty):
            ...
            if "y" in input_str:
                ...
                if not status:
                    await event.edit("أحتاج إلى امتيازات المشرف للقيام بهذا الإجراء!")
                    e.append(str(e))
                    break
                else:
                    c = c + 1
        ...
        if i.bot:
            ...
            n = n + 1
    if input_str:
        required_string = """تم طرد {} / {} من المستخدمين
الحسابات المحذوفة: {}
حالة المستخدم فارغة: {}
حالة المستخدم الشهر الماضي: {}
حالة المستخدم الأسبوع الماضي: {}
حالة المستخدم في وضع عدم الاتصال: {}
حالة المستخدم عبر الإنترنت: {}
حالة المستخدم مؤخرًا: {}
بوتات: {}
لا يوجد: {}"""
        await event.edit(required_string.format(c, p, d, y, m, w, o, q, r, b, n))
        await asyncio.sleep(5)
    await event.edit("""الإجمالي: {} مستخدمًا
الحسابات المحذوفة: {}
حالة المستخدم فارغة: {}
حالة المستخدم الشهر الماضي: {}
حالة المستخدم الأسبوع الماضي: {}
...
حالة المستخدم مؤخرًا: {}
بوتات: {}
لايوجد: {}""".format(p, d, y, m, w, o, q, r, b, n))
