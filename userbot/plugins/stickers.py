...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        await event.edit(f"  {sticker_set.set.count} () . {Config.TMP_DOWNLOAD_DIRECTORY} {sticker_set.set.short_name} ...")
        ...
        while 1:
            ...
            try:
                await event.edit(
                    f"  {num_tasks - len(pending_tasks)}/{sticker_set.set.count}")
...
def progress(current, total):
    logger.info(" : {}  {} \ n  {}".format(current, total, (current / total) * 100))
